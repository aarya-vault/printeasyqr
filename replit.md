...
- **Shop Status Management System**: Clean, professional feature allowing shop owners to control their shop's availability. Shop owners can open or close their shop at any time using the isOnline master switch. Manual toggle takes precedence over working hours for shop owner dashboard display - shows OPEN when manually set to online (regardless of working hours), CLOSED only when manually set to offline. Customer-facing interfaces show unified status based on both manual override and working hours. The status toggle in the shop dashboard provides instant control with clear visual feedback using standard open/closed indicators.
...
- **Queue Number System & Random Public IDs**: Comprehensive rebranding from "dynamic order number" to "queue number" across all interfaces for improved user clarity. Implemented dual ID system with sequential internal IDs for database operations and random alphanumeric public IDs (e.g., "ORD-X7K9M2P") for customer-facing displays. Enhanced security by preventing business metric exposure while maintaining backward compatibility. Queue numbers automatically reset when all orders are completed, providing accurate position tracking for customers.
- **Enhanced JWT Authentication Flow**: Integrated just-in-time authentication in anonymous order flow with automatic user creation/lookup during order placement. JWT tokens generated seamlessly for dashboard access with extended validity (90 days). Enhanced auth context with optimized token validation, automatic refresh, and proper cleanup. Improved user experience from order creation to tracking without additional login steps.
- **Advanced Order Management**: Implemented sophisticated queue calculation logic considering only active orders (new, pending, processing, ready) while excluding completed and soft-deleted orders. Enhanced order transformation with dual ID support, proper data sanitization, and comprehensive error handling. Added role-based order creation safeguards preventing shop owners from ordering from their own shops.
- **Complete Database Migration (August 16, 2025)**: Successfully migrated from external Neon database to Replit's integrated PostgreSQL system. Imported complete production dataset with 107 authentic print shops, 85 users (84 shop owners, 1 customer), 9 orders with file attachments, and 3 real-time chat messages. All data integrity maintained including R2 file storage keys, user authentication, and business relationships. Added system administrator account for platform management.
- **Critical Shop Management Data Loss Fix (August 17, 2025)**: Identified and resolved catastrophic bug where shop name updates created duplicate shop records instead of updating existing ones, causing all order data to vanish. Root cause was accidental use of Google Maps import route (designed for creating new shops) instead of update endpoints. Implemented bulletproof safeguards to prevent duplicate shop creation and restored 6 orphaned orders from shop_id=2 to current shop_id=87. Added comprehensive duplicate detection in Google Maps import route with clear error messages directing users to proper update endpoints.
- **File Upload & Confirmation System Fix (August 17, 2025)**: Fixed critical file upload confirmation process where orders showed empty file arrays despite successful R2 uploads. Enhanced error logging in direct-upload.ts to show detailed confirmation results. Confirmed working file attachment system via API testing (Order 13 successfully confirmed with test file). Fixed React hooks order error in shop-order.tsx by moving useMemo before conditional returns. Updated shop dashboard status button colors to use unified green/red system.
- **Shop Status Display Logic Fix (August 17, 2025)**: Unified shop status logic across backend and frontend. Manual override (isOnline) now takes full precedence over working hours - shops show as OPEN when manually toggled online regardless of working hours, and CLOSED only when manually set offline. Fixed backend calculateUnifiedStatus to match frontend expectations.
- **Order Confirmation Redirect Fix (August 17, 2025)**: Simplified order confirmation page redirect logic to reliably navigate customers to their dashboard after countdown timer. Removed complex role detection logic since order confirmation is only accessed by customers. Used direct window.location.href for most reliable navigation.
