...
export default function BeautifulShopDashboard() {
  ...
  // Sort orders by oldest first and add sequence numbers
  const sortedOrders = [...orders].sort((a, b) => 
    new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime()
  );

  // Add sequence numbers to orders
  ...
  const filteredOrders = ordersWithSequence.filter(order => {
                ...
                try {
                  ...
                  return files.map((file: any, index: number) => (
                    <div key={index} className="flex items-center justify-between p-2 bg-gray-50 rounded text-xs">
                      <span className="truncate flex-1 mr-2" title={file.originalName || `file_${index + 1}`}>
                        {file.originalName || `file_${index + 1}`}
                      </span>
                      <div className="flex items-center space-x-1">
                        ...
                        <Button
                          ...
                          variant="ghost"
                          className="h-6 w-6 p-0"
                          onClick={() => handleDownloadFile(file, file.originalName || `file_${index + 1}`)}
                        >
      ...
      <div className="bg-white border-b shadow-sm sticky top-0 z-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex items-center space-x-3">
              {/* Toggle Switch for Shop Status */}
              <div className="flex items-center space-x-2">
                <label htmlFor="shop-status" className="text-sm font-medium text-gray-700">
                  {shopData.shop.isOnline ? 'Accepting Orders' : 'Offline'}
                </label>
                <div className="relative">
                  <input
                    id="shop-status"
                    type="checkbox"
                    checked={shopData.shop.isOnline}
                    onChange={() => toggleShopStatus.mutate()}
                    disabled={toggleShopStatus.isPending}
                    className="sr-only"
                  />
                  <div 
                    onClick={() => !toggleShopStatus.isPending && toggleShopStatus.mutate()}
                    className={`block w-14 h-8 rounded-full cursor-pointer transition-colors ${
                      shopData.shop.isOnline ? 'bg-green-500' : 'bg-gray-300'
                    } ${toggleShopStatus.isPending ? 'opacity-50 cursor-not-allowed' : ''}`}
                  >
                    <div className={`absolute left-1 top-1 bg-white w-6 h-6 rounded-full transition-transform ${
                      shopData.shop.isOnline ? 'translate-x-6' : ''
                    }`}></div>
                  </div>
                </div>
              </div>
