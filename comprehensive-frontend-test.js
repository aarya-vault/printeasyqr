...
// Tests all functionality from shop application to order completion
...
async function testShopApplication() {
  ...
  const applicationData = {
    ...
    publicContactNumber: "9876543210",
    internalShopName: "Test Internal Shop",
    email: `testshop${Date.now()}@example.com`,
    phoneNumber: `98765${Date.now().toString().slice(-5)}`,
    password: "Test@123",
...
async function testShopOwnerLogin() {
  ...
  // Get application details to get credentials
  const appResult = await apiRequest('GET', `/api/admin/shop-applications`, null, testData.adminToken);
  
  if (!appResult.ok) {
    console.log(`${colors.red}✗ Failed to get applications${colors.reset}`);
    return false;
  }
  
  const application = appResult.data.find(app => app.id === testData.applicationId);
  if (!application) {
  ...
  if (result.ok && result.data.token) {
    ...
    console.log(`${colors.green}✓ Shop owner login successful${colors.reset}`);
    return true;
  } else {
...
async function testGetShopDetails() {
  ...
  if (result.ok && result.data.shop) {
    ...
    console.log(`${colors.green}✓ Shop details retrieved (ID: ${result.data.shop.id})${colors.reset}`);
    return true;
  } else {
...
async function testCustomerRegistration() {
  ...
  if (result.ok && result.data.token) {
    ...
    console.log(`${colors.green}✓ Customer registered successfully${colors.reset}`);
    return true;
  } else {
...
async function testUnreadMessageCount() {
  ...
  const result = await apiRequest('GET', `/api/messages/unread/count?orderId=${testData.orderId}`, null, testData.shopOwnerToken);
  
  if (result.ok) {
    console.log(`${colors.green}✓ Unread count retrieved: ${result.data.count}${colors.reset}`);
    return true;
  } else {
...
async function runTests() {
  ...
  console.log(`${colors.bright}${colors.magenta}COMPREHENSIVE FRONTEND TESTING${colors.reset}`);
  console.log(`${colors.cyan}${'='.repeat(60)}${colors.reset}\n`);
